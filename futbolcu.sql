...
CREATE TABLE IF NOT EXISTS `futbolcu` (
  ...
  `takim` int(11) DEFAULT NULL,
  `mevki` int(11) DEFAULT NULL,
  `forma` tinyint(4) DEFAULT NULL,
  PRIMARY KEY (`sira`),
  KEY `FK_futbolcu_takim` (`takim`),
  KEY `FK_futbolcu_mevki` (`mevki`),
  CONSTRAINT `FK_futbolcu_takim` FOREIGN KEY (`takim`) REFERENCES `takim` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
